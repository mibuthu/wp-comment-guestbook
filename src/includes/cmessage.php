...
class CMessage {
	...
	public function add_cmessage_indicator( $url ) {
		...
		if ( ( $this->config->page_cmessage_enabled->is_true() && ! $is_cgb_comment )
			|| ( $this->config->cmessage_enabled->is_true() && $is_cgb_comment )
		) {
	...
	private function print_script_variables() {
		echo '
			<script type="text/javascript">
				var cmessage_text = "' . wp_kses_post( $this->config->cmessage_text->as_str() ) . '";
				var cmessage_type = "' . wp_kses_post( $this->config->cmessage_type->as_str() ) . '";
				var cmessage_duration = ' . wp_kses_post( $this->config->cmessage_duration->as_str() ) . ';
				var cmessage_styles = "' . wp_kses_post( str_replace( [ '&#10;&#13;', "\r\n", '&#10;', '&#13;', "\r", "\n" ], ' ', $this->config->cmessage_styles->as_str() ) ) . '";
			</script>';
	}
